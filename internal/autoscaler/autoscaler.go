// Package autoscaler implements auto-scaling orchestration for dynamic infrastructure.
package autoscaler
...
out, err := sshutil.RunSSH(nodeIP, cmd)
if err == nil {
...
_, err := virtualbox.RunVBoxQuiet("createvm", "--name", nodeName, "--ostype", "Debian_64", "--register")
if err != nil {
logEvent("Fallo creación: " + err.Error())
return
}

virtualbox.RunVBoxQuiet("modifyvm", nodeName, "--memory", "512", "--nic1", "bridged", "--bridgeadapter1", cfg.NetworkAdapter)
virtualbox.RunVBoxQuiet("storagectl", nodeName, "--name", "SATA", "--add", "sata")
virtualbox.RunVBoxQuiet("storageattach", nodeName, "--storagectl", "SATA", "--port", "0", "--device", "0", "--type", "hdd", "--medium", cfg.DiskUUID, "--mtype", "multiattach")
virtualbox.RunVBoxQuiet("startvm", nodeName, "--type", "headless")

// Store port for dashboard display
virtualbox.RunVBoxQuiet("guestproperty", "set", nodeName, "/Gestor/Port", cfg.AppPort)

ip, err := virtualbox.GetVMIP(nodeName)
if err != nil {
...
// go displayVMStatusTable() removed to prevent circular dependency
}
...
// Then destroy the VM
virtualbox.RunVBoxQuiet("controlvm", nodeName, "poweroff")
time.Sleep(3 * time.Second)
virtualbox.RunVBoxQuiet("unregistervm", nodeName, "--delete-all")
...
sshutil.CreateRemoteFile(lbIp, tempRemotePath, cfg.String())
sshutil.RunSSH(lbIp, "sudo systemctl restart haproxy")
}
...
w.Header().Set("Content-Type", "application/json")
json.NewEncoder(w).Encode(api.Response{Success: true, Message: "Configuración de AutoScaling guardada"})
}
}
